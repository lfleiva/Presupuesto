...
public class AccesoDatos {
    ...
    public <E> List<E> consultarObjetoPorVigencia(Class<E> clase, Object objeto, Vigencia vigencia) {
        entityManager = null;
    ...
    public <E> List<E> consultarTodosPorVigencia(Class<E> clase, Vigencia vigencia) {
        entityManager = null;
        ...
        try {
            ...
            // Ensamblar la consulta
            StringBuilder strConsulta = new StringBuilder();
            strConsulta.append("SELECT objeto FROM ").append(clase.getName()).append(" AS objeto WHERE objeto.vigencia = :vigencia");
    ...
    /**
     * Metodo que consulta la vigencia ectiva
     *
     * @return
     */
    public Vigencia consultarVigenciaActiva() {
        Vigencia vigencia = new Vigencia();
        vigencia.setActiva(true);
        vigencia = consultarTodos(vigencia, Vigencia.class).get(0);
        return vigencia;
    }
    ...
    private String doConstruirCondiciones(Object objeto, List<String> listaAtributos, List<Object> listaValores, String prefijo) {
        ...
        for (Method metodo : listaMetodos) {
            ...
            if (metodo.getName().substring(0, 3).equals("get")) {
                ...
                if (metodo.getReturnType().equals(Boolean.class) || metodo.getReturnType().equals(Byte.class) || metodo.getReturnType().equals(Short.class) || metodo.getReturnType().equals(Integer.class) || metodo.getReturnType().equals(Long.class) || metodo.getReturnType().equals(Float.class) || metodo.getReturnType().equals(Double.class) || metodo.getReturnType().equals(String.class) || metodo.getReturnType().equals(Character.class) || metodo.getReturnType().equals(TipoRubro.class) || metodo.getReturnType().equals(BigDecimal.class)) {
    ...
    /*
    public List<RubroXDisponibilidad> consultarRubroXDisponibilidad(RubroXDisponibilidad rubroXDisponibilidad, Vigencia vigencia) {
        List<RubroXDisponibilidad> listaRubroXDisponibilidad = new ArrayList<RubroXDisponibilidad>();

        StringBuilder sql = new StringBuilder();
        sql.append("SELECT RXD FROM RubroXDisponibilidad RXD ");
        sql.append("WHERE RXD.disponibilidad = :disponibilidad ");
        sql.append("AND RXD.vigencia = :vigencia ");
        sql.append("ORDER BY RXD.idRubroXDisponibilidad DESC");

        Map<String, Object> mapParametros = new HashMap<String, Object>();
        mapParametros.put("disponibilidad", rubroXDisponibilidad.getDisponibilidad());
        mapParametros.put("vigencia", vigencia);

        listaRubroXDisponibilidad = consultarHSQL(sql.toString(), mapParametros, RubroXDisponibilidad.class, 0, 0);

        if (listaRubroXDisponibilidad != null && !listaRubroXDisponibilidad.isEmpty()) {
            return listaRubroXDisponibilidad;
        } else {
            return null;
        }
    }*/
    ...
    public void limpiarTablaVigencia(String tabla, Vigencia vigencia) {
        entityManager = null;
        try {
            entityManager = getEntityManager();
            entityManager.getTransaction().begin();
            Query query = entityManager.createNativeQuery("DELETE FROM " + tabla + " WHERE vigencia = " + vigencia.getIdVigencia());
            query.executeUpdate();
            entityManager.getTransaction().commit();
        } finally {
            if (entityManager != null && entityManager.isOpen()) {
