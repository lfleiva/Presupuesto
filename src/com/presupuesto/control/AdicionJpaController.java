...
public class AdicionJpaController implements Serializable {
    ...
    public AdicionJpaController() {}
    
    public AdicionJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public void destroy(Adicion adicion) throws NonexistentEntityException {
        entityManager = null;
        try {
            entityManager = getEntityManager();
            entityManager.getTransaction().begin();            
            try {
                //adicion = em.getReference(Adicion.class, adicion.getIdAdicion());
                adicion.getIdAdicion();
            } catch (EntityNotFoundException enfe) {
                throw new NonexistentEntityException("The adicion with id " + adicion.getIdAdicion() + " no longer exists.", enfe);
            }
            Vigencia vigencia = adicion.getVigencia();
            if (vigencia != null) {
            ...
            for (AdicionRubro adicionRubroListAdicionRubro : adicionRubroList) {
                ...
                adicionRubroListAdicionRubro = entityManager.merge(adicionRubroListAdicionRubro);
            }
            entityManager.remove(entityManager.contains(adicion) ? adicion : entityManager.merge(adicion));
