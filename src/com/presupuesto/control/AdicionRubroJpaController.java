 ...
 */
public class AdicionRubroJpaController implements Serializable {

    private static EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("PresupuestoPU");
    private EntityManager entityManager;

    public AdicionRubroJpaController() {
    }

    public AdicionRubroJpaController(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public void eliminarAdicionRubro(AdicionRubro asicionRubro) {
        
    }
    
    public void destroy(AdicionRubro adicionRubro) throws NonexistentEntityException {
        entityManager = null;
        try {
            ...
            try {
                ...
                throw new NonexistentEntityException("The adicionRubro with id " + adicionRubro.getIdAdicionRubro() + " no longer exists.", enfe);
            }
            Rubro rubro = adicionRubro.getRubro();
            if (rubro != null) {
            ...
            if (adicion != null) {
                ...
                adicion = entityManager.merge(adicion);
            }
            Vigencia vigencia = adicionRubro.getVigencia();
            if (vigencia != null) {
            ...
            entityManager.remove(entityManager.contains(adicionRubro) ? adicionRubro : entityManager.merge(adicionRubro));
